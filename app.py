...
lineaccesstoken = 'imEwOG3YIIBYCdg829SEnVX8u8WO7LD3l65pSNJyIohV6agvIpJrZJb5+HevE+/GV9tn6+BmnuKHPxQizbtFzKWZqgWf7mFeK6Cu7BUT7/iFtpco+f6CCDtRTz7EoiTf4KWVh1BmFYHXkxvHwm1kEwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if msg == "ทำไร" :
            replyObj = TextSendMessage(text="คุยกับคุณอะ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "สวัสดี" :
            replyObj = TextSendMessage(text="เออ....หวัดดีเว้ย")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "กินข้าวยัง" :
            replyObj = TextSendMessage(text="กินเเล้วครับ")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/8e9fa246-089e-4151-a394-92ccd2da97cc"
